...
def main(**kwargs):
    ...
    data_scale = kwargs.get('data_scale', None)
    ...
    if train_config.use_peft:
        ...
        # Generate the peft config and start fine-tuning from original model
            ...
            peft_kwargs = {k.replace('lora_', ''): v for k, v in kwargs.items() if k in ['lora_r', 'lora_target_modules']}
            peft_config = generate_peft_config(train_config, peft_kwargs)
    ...
    if data_scale:
        dataset_train = dataset_train.shuffle(seed=42).select(range(int(data_scale * len(dataset_train))))
    if not train_config.enable_fsdp or rank == 0:
    ...
    #     named_parameters = [k for k, v in model.named_parameters()]
    # state_dict_keys = model.state_dict().keys()
    # if rank==0:
    #     print(f"Named parameters: {named_parameters}")
    #     print(f"State dict: {state_dict_keys}")
    # dist.barrier()
    # exit()
